...
#Vald <- left_join(Vald, positions2, by = 'anon_id')
...
## How Well Riskstatus Predicts Outcome

```{r}
final_merge2$Status <- factor(
  final_merge2$Status,
...
```{r}
model_q1 <- glm(HSI~Status, data=final_merge2, family="binomial")
summary(model_q1)
exp(coef(model_q1))
```

```{r}
final_merge2$pred_prob_q1 <- predict(model_q1,type="response")
final_merge2$pred_class_q1 <- ifelse(final_merge2$pred_prob_q1>0.5,1,0) #class w/ 0.5 threshold

pred <- factor(final_merge2$pred_class_q1, levels = c(0, 1))
actual <- factor(final_merge2$HSI, levels = c(0, 1))
...
```{r}
roc_obj <- roc(final_merge2$HSI, final_merge2$pred_prob_q1)
plot(roc_obj, main="ROC Curve", col="pink",lwd=3)
auc(roc_obj)
```

```{r}
table(final_merge2$HSI)
prop.table(table(final_merge2$HSI))
```
...
model <- glm(HSI ~ ., data = final_set, family = 'binomial')
...
```{r}
dash_2 <- left_join(dash_final, threshs, by = 'Position') %>%
  filter(Position == 'RB' | Position == 'LB' | Position == 'DB') %>%
  mutate(Flag.Imbalance = ifelse(Nordic.Mean.Imbalance > Imbalance.Thresh, 1, 0),
         Flag.Diff.Norm = ifelse(Perc.Diff.Norm < Diff.Norm.Thresh, 1, 0))
```
...
```{r}
c <- Vald_prev %>%
  group_by(HR, Position) %>%
  summarize(Avg.Max.Force = mean(Maximum.Force, na.rm = TRUE),
            Avg.Imbalance = mean(Nordic.MEAN.Imbalance, na.rm = TRUE),
            Avg.Perc.Diff.Norm = mean(Perc.Diff.Norm, na.rm = TRUE),
            count = n())
```

No specialists were deemed high risk    
...
Difference in maximum force between HR and other appears to be statistically significant for TEs, OLs, DBs, LBs, and DLs. For TEs and QBs the average maximum force for those who were deemed high risk is actually higher than that of those not deemed high risk.  
...
t.test(Nordic.MEAN.Imbalance ~ HR, data = RBs)
t.test(Nordic.MEAN.Imbalance ~ HR, data = WRs)
t.test(Nordic.MEAN.Imbalance ~ HR, data = TEs)
t.test(Nordic.MEAN.Imbalance ~ HR, data = OLs)
t.test(Nordic.MEAN.Imbalance ~ HR, data = DBs)
t.test(Nordic.MEAN.Imbalance ~ HR, data = LBs)
...
Threshold Example: RB 9.243539 (upper bound + mean group 0) 
Threshold Example: WR 12.5772 (upper bound + mean group 0)
Threshold Example: DB 12.6257 (upper bound + mean group 0)
Threshold Example: LB 9.93417 (upper bound + mean group 0)

```{r}
Position <- c('QB', 'RB', 'WR', 'TE', 'OL', 'DB', 'LB', 'DL')
Imbalance.Thresh <- c(NA, 9.243539, 12.5772, NA, NA, 12.6257, 9.93417, NA)
...
t.test(Perc.Diff.Norm ~ HR, data = WRs)
t.test(Perc.Diff.Norm ~ HR, data = TEs)
t.test(Perc.Diff.Norm ~ HR, data = OLs)
t.test(Perc.Diff.Norm ~ HR, data = DBs)
t.test(Perc.Diff.Norm ~ HR, data = LBs)
t.test(Perc.Diff.Norm ~ HR, data = DLs)
```
...
Threshold Example: RB -9.382656 (lower bound - mean group 0)
Threshold Example: OL -7.424226 (lower bound - mean group 0)
Threshold Example: DB -15.15471 (lower bound - mean group 0)
Threshold Example: LB -15.4924 (lower bound - mean group 0)

```{r}
Diff.Norm.Thresh <- c(NA, -9.382656, NA, NA, -7.424226, -15.15471, -15.4924, NA)
