...
vald_merge <- Vald %>%
  select(-c('Nordic.Left.Z.Score', 'Nordic.Right.Z.Score')) %>%
...
*Group = ifelse(Date < Injury.1.Start, 0, ifelse(Injury.1.Start <= Date & Date <= Injury.1, 1, ifelse(Injury.2.Start <= Date & Date <= Injury.2, 2, ifelse(Injury.3.Start <= Date & Date <= Injury.3, 3, ifelse(Injury.4.Start <= Date & Date <= Injury.4, 4, 'After')))))*

*ifelse(Injury.1.Start <= Date & Date <= Injury.1, 1, ifelse(Injury.2.Start <= Date & Date <= Injury.2, 2, ifelse(Injury.3.Start <= Date & Date <= Injury.3, 3, ifelse(Injury.4.Start <= Date & Date <= Injury.4, 4, 0))))*
...
merge_perfnorm <- merge_perfnorm %>%
  ...
  summarise(Nordic.Average = mean(Nordic.Average, na.rm = TRUE),
            ...
            Modified.RSI = mean(Modified.RSI, na.rm = TRUE), .groups = 'drop')
```
...
9. Merging in All other Lower Body Injuries
...
10. Merging All Datasets Together
...
```{r}
model_status <- glm(HSI ~ Status, data = final_merge, family = 'binomial')
...
dash <- dash %>%
  ...
  select(anon_id, Prev.Injuries, Prev.HSI, Perc.Diff.Norm, Nordic.Mean.Imbalance, Trend, Status, HSI) %>%
...
ggplot(data = dash_final, aes(x = Prev.Injury.Num, y = HSI)) + 
...
ggplot(data = dash_final, aes(x = Perc.Diff.Norm, y = HSI)) + 
  geom_boxplot(color = 'black', fill = 'royalblue') + 
...
ggplot(data = dash_final, aes(x = Nordic.Mean.Imbalance, y = HSI)) + 
  geom_boxplot(color = 'black', fill = 'royalblue') + 
...
ggplot(data = dash_final, aes(x = Trend, y = HSI)) + 
  geom_boxplot(color = 'black', fill = 'royalblue') + 
