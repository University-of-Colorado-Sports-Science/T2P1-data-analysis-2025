...
filter_full3 <- filter_full3_merge %>%
 group_by(anon_id) %>%
 mutate(force_zscore= scale(Maximum.Force)) %>%
 ungroup() %>%
...
ggplot(data=filter_full3, aes(x = nordic_zscore, y = as.numeric(HSI))) +
  geom_smooth() +
...
#do with all different positions (pay special attention to positions highlgihted on the google doc)
```

```{r}
#then summarizing tables, findings 
...
##Multimetric Matrix- idea with the goal to create a risk score by counting how many thresholds an athlete breaches 
