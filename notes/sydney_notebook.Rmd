...
merge_incid <- merge_incid %>%
  ...
  summarise(Body.Part,OSICS14.Code = paste0(OSICS14.Code, collapse = " and ")) %>%
...
#merge_position <- merge_position %>%
 # group_by(anon_id, Group) %>%
 # summarise(OSICS14.Code, Body.Part=get_mode(Body.Part),Position = get_mode(Position), .groups = 'drop')
...
#merge_position <- merge_position %>%
  #select(-c(OSICS14.Code)) #%>%
  #group_by(anon_id) %>%
 # summarize(Body.Part,Position, HSI)
```
...
#adding positions to final dataset 
...
filter_full3_merge <- full3_merge %>%
  #summarise(Status= Status) %>%
...
#logistic regression
model_q1 <- glm(HSI~Status, data=filter_full3_merge, family="binomial")
summary(model_q1)
...
library(caret)
#model evaluation
filter_full3_merge$pred_prob_q1 <- predict(model_q1,type="response")
filter_full3_merge$pred_class_q1 <- ifelse(filter_full3_merge$pred_prob_q1>0.5,1,0) #class w/ 0.5 threshold

#convert both predicted + actual to factors w/ the same levels
pred <- factor(filter_full3_merge$pred_class_q1, levels = c(0, 1))
actual <- factor(filter_full3_merge$HSI, levels = c(0, 1))
...
library(pROC)
#ROC CURVE
roc_obj <- roc(filter_full3_merge$HSI, filter_full3_merge$pred_prob_q1)
...
set.seed(22)
balanced <- upSample(x=filter_full3_merge %>% dplyr::select(Status), 
         y= as.factor(filter_full3_merge$HSI))

model_bal <- glm(Class~ Status, data=balanced, family="binomial")
summary(model_bal)

#ordinal logistic rregress. // treat outcome as predict risk status (reverse logic)
model_ord <- polr(Status~HSI, data=filter_full3_merge, method="logistic")
summary(model_ord)
```
...
```{r}
##DELTE PROBABLY 
#order watch list category 
final_merge_full$Status <- factor(final_merge_full$Status,ordered=TRUE,levels=c("routine","low","medium", "high"))
```

```{r}
#calculate averages to base risk flags off of 
mean(filter_full3_merge$Nordic.MEAN.Imbalance.with.side)
mean(filter_full3_merge$Percent.Difference.Norm)
...
filter_full3_merge <- filter_full3_merge %>%
  mutate(nordic_imbalance_flag= if_else(Nordic.MEAN.Imbalance.with.side>1.28,1,0),
         difference_norm_flag= if_else(Percent.Difference.Norm>-5.93,1,0),
         time_injured_flag= if_else(Time.Injured> 13.25,1,0),
         nordic_z_self= scale(Maximum.Force))
```

Using SMOTE Sampling for ClasS Imbalance Issues
##NEED TO FIGURE OUT HOW THIS WORKS
```{r}
library(smotefamily)
filter_full3_merge$HSI <- as.factor(filter_full3_merge$HSI)

#application of SMOTE to balance classes 
balanced_data <- SMOTE(filter_full3_merge[,-16],filter_full3_merge[16],k=2, dup_size = 0)
table(balanced_data$HSI)
```

```{r}
#training and testing 
train_idx <- createDataPartition(balanced_data$HSI, p=.80, list=FALSE)
...
null_model_watchlist <- lm(Status~NULL, data=final_merge_full)
summary(null_model_watchlist)

#fit null model for HSI
null_model_HSI <- lm(HSI~NULL, data=filtered_full3_merge)
summary(null_model_HSI)

#NEED to turn recurrence words into # groups 

#fit null model for subsequent HSI injury 
null_model_recurrence <- lm(Recurrence~NULL, data=filtered_full3_merge)
summary(null_model_recurrence )
```

```{r}
#convert all categorical variables to factors 
final_merge_full$anon_id <- as.factor(final_merge_full$anon_id)
final_merge_full$Status <- as.factor(final_merge_full$Status)
final_merge_full$Group <- as.factor(final_merge_full$Group)
final_merge_full$Body.Part <- as.factor(final_merge_full$Body.Part)
final_merge_full$Position <- as.factor(final_merge_full$Position)
final_merge_full$OSICS14.Code <- as.factor(final_merge_full$OSICS14.Code)
final_merge_full$HSI <- as.factor(final_merge_full$HSI)
```


Highly unbalanced dataset for predction methods of HSI.
```{r}
#upsample to even out classes of HSI more 
library(dplyr)
#separate the classes
majority <- filtered_full3_merge %>% filter(HSI == 0)
minority <- filtered_full3_merge %>% filter(HSI == 1)

#sample from majority to match minority
set.seed(22)
majority_down <- majority %>% sample_n(nrow(minority) * 2)  #ratio
balanced_data <- bind_rows(minority, majority_down)
```
...
```{r}
library(randomForest)
rf<- randomForest(as.factor(HSI)~., data=final_merge_full)
varlmpPlot(rf)
```
