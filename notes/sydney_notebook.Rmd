...
Q2.
How do components of the watch list predict HSI?
subsequent//following HSI injury risk?
...
start1 <- start %>%
  na.omit()    #143obs.
perf <- subset(perfnorm, select=c(anon_id,Eccentric.Peak.Power...Relative))
...
vaLd <- vAld %>%
  ...
  summarise(Avg.Nordic.Left.Z.Score= mean(Nordic.Left.Z.Score, na.rm=TRUE),
           ...
           nordic_imbalance_score= mean(Nordic.MEAN.Imbalance.with.side, na.rm=TRUE)) %>%
  mutate(nordic_imbalance_score=scale(nordic_imbalance_score))
...
thresh_final <- thresh_data_final %>%
  mutate(nordic_imbalance_flag= if_else(nordic_imbalance_score<-1.28|nordic_imbalance_score>1.28,1,0),
...
metrics <- c("Avg.Nordic.Left.Z.Score","Avg.Nordic.Right.Z.Score","nordic_zscore", "force_zscore", "recurrence_flag", "mechanism_flag","nordic_imbalance_flag")
...
#each component contributes either 0 or 1, total risk score==range from 0 to 5
...
##OLD CODE WITH WORSE DATASET BUT WORKS // TRYING A NEW ONE
...
#Q3 ##THRESHOLD EXPLORATION Q3: Can we find better thresholds for flagging risks?
