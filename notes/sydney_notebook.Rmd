...
#1512 observations and 58 variables 
```
...
filtered_full3_merge <- full_merge %>%
...
cor(full_merge  %>% select_if(is.numeric))
```
...
ordinal_model <- polr(Status~HSI, data=final_merge_full, method="logistic")
...
null_model_HSI <- lm(HSI~NULL, data=final_merge_full)
summary(null_model_HSI)

#fit null model for subsequent HSI injury 
#null_model_HSIx2 <- lm(HSI~NULL, data=final_merge_full)
#summary(null_model_HSIx2)
```
```{r}
...
majority <- final_merge_full %>% filter(HSI == 0)
minority <- final_merge_full %>% filter(HSI == 1)

#sample from majority to match minority
set.seed(22)
majority_down <- majority %>% sample_n(nrow(minority) * 5)  #ratio
balanced_data <- bind_rows(minority, majority_down)

#fit full logistic model
model <- glm(HSI~., data=balanced_data, family="binomial")
summary(model)
...
```{r}
model2 <- glm(HSI~Positional.Norm+Maximum.Velocity+Maximum.Force+ Explosive.Efforts +Modified.RSI+Nordic.Imbalance.., data=final_merge_full, family="binomial")
summary(model2)
...
#nonlinear model attempt as data is known to be nonlinear // Random Forests 
