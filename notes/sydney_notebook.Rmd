...
#OF 1703 observations 28 of them an HSI injury occured 
```
...
count_of_ones_DB_high <- sum(filtered_DB$Highrisk_binary)
count_of_ones_DB_hsi <- sum(filtered_DB$HSI)

count_of_ones_DB_high # number of DB deemed high risk
count_of_ones_DB_hsi # number of DB w/ an HSI injury 
```
...
count_of_ones_DL_high <- sum(filtered_DL$Highrisk_binary)
count_of_ones_DL_hsi <- sum(filtered_DL$HSI)

count_of_ones_DL_high # number of DL deemed high risk
count_of_ones_DL_hsi # number of DL w/ an HSI injury 
```
```{r}
#find rate of high risk per position also rate of HSI per position 
#QUARTER BACK 
...
count_of_ones_QB_high <- sum(filtered_QB$Highrisk_binary)
count_of_ones_QB_hsi <- sum(filtered_QB$HSI)

count_of_ones_QB_high # number of QB deemed high risk
count_of_ones_QB_hsi # number of QB w/ an HSI injury 
```
```{r}
#find rate of high risk per position also rate of HSI per position 
#WIDE RECIEVER 
...
#1782 observations and 58 variables 
```
...
#Positional.Norm, Nordic.MEAN.Imbalance.with.side(VALD), 
#VALD
...
#full <- merge(full3_merge, merge_catapult2, by = c('anon_id', 'Group'), all = TRUE) #ADDED
...
#Left.Hamstring.Force..Nordic. //BAD CHANGE
...
```{r}
#Stepwise validation 
table(filter_full3_merge$Status,filter_full3_merge$HSI)

table(final_merge_full$Status,filter_full3_merge$HSI)
```

```{r}
#ordinal variable 
filter_full3_merge$Status <- factor(
  filter_full3_merge$Status,
...
filter_full3_merge <- filter_full3_merge %>%
...
filter_full3 <- filter_full3_merge %>%
...
#logistic regression
model_q1 <- glm(HSI~Status, data=filter_full3, family="binomial")
summary(model_q1)
...
library(caret)
#model evaluation
filter_full3$pred_prob_q1 <- predict(model_q1,type="response")
filter_full3$pred_class_q1 <- ifelse(filter_full3$pred_prob_q1>0.5,1,0) #class w/ 0.5 threshold

#convert both predicted + actual to factors w/ the same levels
pred <- factor(filter_full3$pred_class_q1, levels = c(0, 1))
actual <- factor(filter_full3$HSI, levels = c(0, 1))
...
library(pROC)
#ROC CURVE
roc_obj <- roc(filter_full3$HSI, filter_full3$pred_prob_q1)
plot(roc_obj, main="ROC Curve", col="pink",lwd=3)
auc(roc_obj)
```

## 6 MONTH VALIDATION//COMPLETE
...
table(filter_full3_merge$HSI)
prop.table(table(filter_full3_merge$HSI))
```
...
mean(filter_full3_merge$Nordic.MEAN.Imbalance.with.side)
mean(filter_full3_merge$Maximum.Force)
mean(filter_full3_merge$Nordic.Right.MAX)
mean(filter_full3_merge$Nordic.Left.MAX)
mean(filter_full3_merge$Overall.Wellness.Score)

#convert from character to numeric
filter_full3_merge$Time.Injured <- as.numeric(filter_full3_merge$Time.Injured)
mean(filter_full3_merge$Time.Injured)
```

```{r}
#BINARY FLAGS FOR RISK
filter_full3 <- filter_full3 %>%
 # group_by(anon_id) %>%
 # mutate(maxforce_z_self= scale(Maximum.Force)) %>%
     #   ungroup() %>%
  mutate(nordic_imbalance_flag= if_else(Nordic.MEAN.Imbalance.with.side>1.28,1,0),
         nordic_z_right= scale(Nordic.Right.MAX),
         nordic_zleft= scale(Nordic.Left.MAX),
         time_injured_flag= if_else(Time.Injured> 13.25,1,0))

#convert all categorical variables to factors 
filter_full3$anon_id <- as.factor(filter_full3$anon_id)
filter_full3$Status <- as.factor(filter_full3$Status)
filter_full3$HSI <- as.factor(filter_full3$HSI)
#filter_full3$Recurrence <- as.factor(filter_full3$Recurrence)
filter_full3$Trend <- as.factor(filter_full3$Trend)
...
##ALL THE NULL MODELS
```{r}
#fit null model for risk status on watch list
null_model_watchlist <- lm(Status~NULL, data=train_up)
#summary(null_model_watchlist)

#fit null model for HSI
null_model_HSI <- lm(Class~NULL, data=train_up)
summary(null_model_HSI)

#fit null model for subsequent HSI injury 
null_model_recurrence <- lm(Recurrence~NULL, data=train_up)
summary(null_model_recurrence)
```

##Basic Models
```{r}
#
larger_model2 <- glm(HSI~Positional.Norm+Maximum.Velocity+Maximum.Force+ Explosive.Efforts +Modified.RSI+Nordic.Imbalance.., data=final_merge_full, family="binomial")
summary(larger_model2)

smaller_model2 <- glm(HSI~Maximum.Force+Percent.Difference.Norm+ Nordic...Difference.from.First.Test, data=filtered_full3_merge, family="binomial")
summary(smaller_model2)
#model3 <- glm(HSI~.-Status-Body.Part-Position-OSICS14.Code, data=final_merge_full, family="binomial")
#summary(model3)

#model_risk2 <- glm(Status~., data=final_merge_full, family="binomial")
```
...
rf_model <- randomForest(as.factor(HSI)~., data=filter_full3, ntree=500, importance=TRUE)
varlmpPlot(rf)

#testing
rf_preds <- predict(rf_model, test, type="prob")[,2]
roc_rf <- roc(test$HSI, rf_preds)
plot(roc_rf)
auc(roc_rf)
```
...
tree_model <- rpart(HSI~., method="class", cp=0.01)

rpart.plot(tree_model,extra=106)

#testing
tree_preds <- predict(tree_model, test, type="prob")[,2]
roc_tree <- roc(test$HSI, tree_preds)
plot(roc_tree)
auc(roc_tree)
```

##XG Boosting -powerful machine learning method
```{r}
library(xgboost)
train_xgb <- train_up %>%
  mutate(HSI= as.numeric(as.character(HSI)))
test_xgb <- test %>%
  mutate(HSI= as.numeric(as.character(HSI)))

#matrix inputs
dtrain <- xgb.DMatrix(data = as.matrix(train_xgb[, c("nordic_z_self", "avg_imbalance", "position_norm_diff")]),
                      label = train_xgb$hsi_incident)
dtest <- xgb.DMatrix(data = as.matrix(test_xgb[, c("nordic_z_self", "avg_imbalance", "position_norm_diff")]),
                     label = test_xgb$hsi_incident)

#train model
xgb_boost_model <- xgbboost(data=dtrain,objective="binary:logistic", nrounds=100,verbose=0)
#testing
pred_xgb <- predict(xgb_boost_model, dtest)
roc_xgb <- roc(test_xgb$HSI,pred_xgb)
plot(roc_xgb)
auc(roc_xgb)
```

```{r}
#track specific athletes through injury + recovery) ex: anon_id[220,83]

#high vs routine risk vs actual injury 
```


##Threshold Optimzation 
Q3: Can we find better thresholds for flagging risks?
```{r}
#boxplots//violin plots
#nordic zscore vs HSI
ggplot(data=, x=HSI, y=nordic_z_self)+
  geom_violin(fill="pink")+
  geom_boxplot(width=0.2)+
  labs(title="Nordic Z-Score vs HSI Outcome")

#Imbalance 
ggplot(data=, aes(x = hsi_incident, y = avg_imbalance)) +
  geom_violin(fill = "pink") +
...
library(rpart)
library(rpart.plot)
#determine threshold w/ decision tree// CART Model

tree_model2 <- rpart(HSI~., data=data, method="class", cp=0.01)
rpart.plot(tree_model2, extra=106)

#test binary flags from these thresholds // rule based features 
data <- data %>%
  #same flags created before
```

```{r}
#logistic regression with flags
model_thresh <- glm(HSI~., data=data, family="binomial")
summary(model_thresh)

#performance of model 
thresh_preds <- predict(model_thresh, type="response")
roc_thresh <- roc(data$HSI, data$thresh_preds)
plot(roc_thresh)
auc(roc_thresh)

#compare rule based risk to watch list(dashboard)
table(data$Status, data$HSI)
table(data$nordic_flag, data$HSI)
```

```{r}
#old vs new classification method
model_watchlist <- glm(HSI~Status,data=data, family="binomial")
model_flags <- glm(HSI~flags set, data=data, family="binomial")

#AUC's
auc_watchlist <- roc(data$HSI, predict(model_watchlist, type="response"))
auc_flags <- roc(data$HSI, predict(model_flags, type="response"))

plot(auc_watchlist,col,col="pink")
lines(auc_flags, col="blue")
legend("bottomright", legend=c("Watch List","Flag Model"), col=c("pink","blue"),lwd=2)
```
...
##Multimetric Matrix
```{r}
#goal to create a risk score by counting how many thresholds an athlete breaches 
...
## Further Validation & Comparision  
