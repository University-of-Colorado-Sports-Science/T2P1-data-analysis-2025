...
train_idx <- createDataPartition(filter_full3_merge$HSI, p=.80, list=FALSE)
train <- filter_full3_merge[train_idx,]
test <- filter_full3_merge[-train_idx,]
...
rose_train <- ROSE(HSI~Maximum.Force+Nordic.MEAN.Imbalance.with.side+nordic_imbalance_flag+difference_norm_flag+time_injured_flag, data= train, seed=1)$data 
...
set.seed(22)
#ROSE model
rose_model <- train(Class~Maximum.Force+Nordic.MEAN.Imbalance.with.side+nordic_imbalance_flag+difference_norm_flag+time_injured_flag, data=rose_train, method="glm", family="binomial", trControl= ctrl, metric="ROC")
...
##NULL MODELS
```{r}
##DO I USE train, train_up or rose_train for these
#fit null model for risk status on watch list
null_model_watchlist <- lm(Status~NULL, data=train)
summary(null_model_watchlist)

#fit null model for HSI
null_model_HSI <- lm(HSI~NULL, data=train)
summary(null_model_HSI)

#NEED to turn recurrence words into # groups (First Recurrence, New Injury/Illness, None)
#None=0, New Injury/Illness=1, First Recurrence=2

#fit null model for subsequent HSI injury 
null_model_recurrence <- lm(Recurrence~NULL, data=train)
summary(null_model_recurrence )
```

```{r}
#convert all categorical variables to factors 
filter_full3_merge$anon_id <- as.factor(final_merge_full$anon_id)
filter_full3_merge$Status <- as.factor(final_merge_full$Status)
...
filter_full3_merge$HSI <- as.factor(final_merge_full$HSI)
```
