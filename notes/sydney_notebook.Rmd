...
Contacthit_binary=ifelse(filter_full3_merge$Mechanism=="Contact",1,0)  #1 is a risk for flag contact sport 
filter_full3_merge$Contacthit_binary = Contacthit_binary
#binary could be useful when testing accuracy/importance of high risk status
Highrisk_binary=ifelse(filter_full3_merge$Status=="High",1,0)
filter_full3_merge$Highrisk_binary = Highrisk_binary

#Z SCORES AND FLAGS FOR RISK and to determine thresholds off of 
filter_full3 <- filter_full3_merge %>%
 group_by(anon_id) %>%
  mutate(nordic_imbalance_flag= if_else(Nordic.MEAN.Imbalance.with.side>1.28,1,0),  #greater then the average then concerning 
         nordic_z_right= scale(Nordic.Right.MAX),
         nordic_z_left= scale(Nordic.Left.MAX),
         nordic_zscore=mean(nordic_z_left+nordic_z_right),
         force_zscore= scale(Maximum.Force),
         ...
         recurrence_flag=if_else(Recurrence>1,1,0))  #flag goes up, as its cocerning aka 2nd injury 
         mechanism_flag=if_else(Mechanism>=1,1,0)   #flag goes up for contact sport

         #nordic zscore, force zscore, trend flag, recurrence flag, ALREADY HAVE highrisk_binary
...
```{r}
#boxplots for visual comparison                     Highrisk_binary== high risk flag  
#boxplot(nordic_zscore~ Highrisk_binary, data=filter_full3, main="Nordic Z-score by High Risk Status")

ggplot(data=filter_full3, aes(x=Status, y=nordic_zscore))+
  geom_smooth() +
  geom_boxplot(width=2)+
  labs(title="Nordic Z-score by High Risk Status")

ggplot(data=filter_full3, aes(x=HSI, y=nordic_zscore))+
  geom_violin(fill="pink")+
  geom_boxplot(width=2)+
  labs(title="Nordic Z-Score vs HSI Outcome")

#Imbalance 
ggplot(data=filter_full3, aes(x =HSI, y = nordic_imbalance_flag)) +
  geom_violin(fill = "pink") +
  geom_boxplot(width = 0.1)

ggplot(data=filter_full3, aes(x = nordic_zscore, y = HSI)) +
  geom_smooth() +
  geom_boxplot(width = 3)+
  labs(title="Injury Rate vs Nordic Z-score")
#see if plots can look better 
```
```{r}
#t tests
t.test(nordic_zscore~Highrisk_binary,data=filter_full3)
t.test(force_zscore~Highrisk_binary,data=filter_full3)  # do again with force (Maximum.Force)
t.test(nordic_z_right~Highrisk_binary,data=filter_full3)
t.test(nordic_z_left~Highrisk_binary,data=filter_full3)
t.test(Overall.Wellness.Score~Highrisk_binary,data=filter_full3)
```
...
aggregate(nordic_zscore~Status,data=filter_full3, mean)
aggregate(force_zscore~Status,data=filter_full3, mean)

#quantiles can be used for thresholds aswell 
quantile(filter_full3$nordic_zscore[filter_full3$Status=="High"],probs=c(0.10,0.25,0.50,0.75))
```

```{r}
#merge position into filter_full3 with HSIincidentvsrisk2 or merge_position 
df <- merge(filter_full3, merge_position, by="anon_id")
df2 <- unique(df)
df1 <- merge(filter_full3, player_byposition, by="anon_id")
```

```{r}
#now concern of different flagging (flags) needed for different positions 
#compare with each different positions 
wr <- subset(data=df2, Position="WR")
boxplot(nordic_zscore~Highrisk_binary, data=wr, main="WR Nordic by Risk")
```
...
metrics <- c("nordic_z_right","nordic_z_left","nordic_zscore", "force_zscore","time_injured_flag","trend_flag", "recurrence_flag", "mechanism_flag","nordic_imbalance_score")

for (pos in positions){
    pos_data <- subset(df1, Position == pos)
...
```{r}
df11 <- df1 %>%
  mutate(risk_score = nordic_imbalance_flag+ recurrence_flag+time_injured_flag+trend_flag)
#then logistic model
model_score <- glm(HSI ~ risk_score, data = df11, family = "binomial")
summary(model_score)

# Plot injury probability by score
ggplot(df11, aes(x = as.factor(risk_score), fill = HSI)) +
  geom_bar(position = "fill") +
  labs(title = "HSI Rate by Threshold Risk Score", y = "Proportion Injured")

#Export Athletes Who Trigger Flags thru risk score 
flagged_athletes <- df11 %>%
  filter(risk_score >= 2) %>%
  mutate(
    nordic_z_right = as.numeric(nordic_z_right),
    nordic_z_left = as.numeric(nordic_z_left)
  ) %>%
  select(anon_id, nordic_z_right, nordic_z_left, Position, Status, risk_score)
...
##BELOW THIS DIDNT WORK WELL // DONT USE 
```{r}
...
##Decision Tree for Threshold Exploration 
```{r}
library(rpart)
library(rpart.plot)
set.seed(22)

tree_model <- rpart(HSI~., data=filter_full3_merge, method="class", cp=0.01)
...
roc_tree <- roc(test$Class, tree_preds)
plot(roc_tree)
auc(roc_tree)
```
...
#matrix inputs
dtrain <- xgb.DMatrix(data = as.matrix(train_xgb[, c("nordic_zscore", "force_zscore")]),
                      label = train_xgb$HSI)
dtest <- xgb.DMatrix(data = as.matrix(test_xgb[, c("nordic_zscore","force_zscore")]),
                     label = test_xgb$HSI)
