...
In those not deemed high risk there are 235 athletes and only 12 unique athletes suffered HSI (0.05106383).

In the whole population there are 268 athletes and only 34 unique instances of HSI which is (0.1268657). 
...
ggplot(LBs, aes(Nordic.MEAN.Imbalance, fill = as.factor(HR))) + 
  geom_histogram(position = 'identity', alpha = 0.5, bins = 25) + 
  scale_fill_manual(values = c('black', '#CFB87C')) + 
  labs(title = 'Nordic Imbalance vs High Risk Status (All Time) for LBs',
...
```{r, warning = FALSE}
#creates a histogram of nordic mean imbalance for WRs filled by high risk status, position = 'identity' argument makes it so the bars do not stack and the vertical line is the example threshold found above
ggplot(WRs, aes(Perc.Diff.Norm, fill = as.factor(HR))) + 
  geom_histogram(position = 'identity', alpha = 0.5, bins = 25) + 
  scale_fill_manual(values = c('black', '#CFB87C')) + 
