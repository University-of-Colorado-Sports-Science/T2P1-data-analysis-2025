...
perfnorm_byposition <- read_csv('data-sets/data-sets-uncompressed/Positional Performance Normative.csv')
player_byposition <- read_csv('data-sets/data-sets-uncompressed/deid_pos.csv')
```
...
perfnorm_byposition2 <- perfnorm_byposition %>%
  transmute(Position, Nordic.Average.Norm = `Nordic Average`)


#merges the VALD dataset to the perfnorm_byposition2 dataset with Nordic Average Norms
Vald <- merge(Vald, perfnorm_byposition2, by = c('Position'), all = TRUE)
...
filtered_wellness <- subset(wellness, select=c("anon_id","Position","Date","Mental.Score","Physical.Score", "Sleep.Quality.Score","Soreness.Score","Overall.Wellness.Score","Wellness.Compliance","Month","Year"))
...
Wellness_merge <- filtered_wellness %>%
  dplyr::select(-c('Month', 'Year', 'Wellness.Compliance')) %>%
  mutate(Date = as.Date(Date, format = '%m/%d/%Y')) %>%
  na.omit()
```
...
## Merging Datasets by Date in Relation to HSI


### Creating a HSI dataset to group observations by date
...
### 10. Merging in All other Lower Body Injuries

```{r}
#merges hamstring injuries with other lower body injuries
merge_lower1 <- merge(HSI_last_injury, incident_lower, by = c('anon_id'), all = TRUE)

#creates a variable called Group that groups based on date within injury intervals and outputs 'After' if the date is after all injuries, and then these are omitted
merge_lower1 <- merge_lower1 %>%
  mutate(Group = case_when(
    is.na(Injury.1) | Date < Injury.1.Start ~ '0',
    !is.na(Injury.1) & Date >= Injury.1.Start & Date < Injury.1 ~ '1',
    !is.na(Injury.2) & Date >= Injury.2.Start & Date < Injury.2 ~ '2',
...
final_merge_full <- merge(final_merge_full, merge_incid, by=c('anon_id','Group'), all=TRUE)
...
```{r}
merge_vald2 <- merge(HSI_3_months, vald_merge, by = c('anon_id'), all = TRUE)

merge_vald2 <- merge_vald2 %>%
...
merge_vald2 <- merge_vald2 %>%
  group_by(anon_id, Group) %>%
  summarise(Maximum.Force = mean(Maximum.Force, na.rm = TRUE),
            Percent.Difference.Norm = mean(Perc.Diff.Norm, na.rm = TRUE),
            Nordic.Left.MAX = mean(Nordic.Left.MAX, na.rm = TRUE),
...
merge_vald2 <- merge_vald %>%
  dplyr::select(-"Percent.Difference.Norm")
```
...
boxplot(Perc.Diff.Norm~Status,data=final_merge_full,lmain="Risk Status vs % Difference Norm", col="lightblue")
...
# Question 1: Validating Soft Tissue Watchlist
...
ggplot(length_time1, aes(x = reorder(anon_id, Test.to.HSI), y = Test.to.HSI)) + 
  geom_col(fill = '#CFB87C', color = 'black', position = "identity", alpha = 0.75) +
  labs(title = 'Time from Test to HSI (High Risk)',
       x = 'Player Anon ID',
       y = 'Days After Being Deemed High Risk') + 
  geom_text(aes(label = Test.to.HSI), vjust = -0.5, size = 2.5) + 
  theme_bw()
```
...
# Question 2. How do components of the watch list predict HSI? subsequent//following HSI injury risk?
#Q3
...
plot1 <- ggplot(thresh_final1, aes(x = Position, fill = risk_score)) +
  geom_bar(position = "stack") +
  scale_fill_manual(
    values = c("1"="#CFB87C","2"="lightyellow","3"="white","4"="#000000", "5"="#565A5C","6"="#A2A4A3")) +
  labs(
    title = "Athletes by Position + Risk Scores",
    x = "Positions",
    y = "Count",
    fill = "Risk Score"
)
...
plot2 <- ggplot(thresh_final, aes(x = Position, fill = HSI)) +
  geom_bar(position = "stack") +
  scale_fill_manual(values=c("1"= "#A2A4A3", "0"= "#CFB87C")) +
  labs(
...
plot3 <- ggplot(thresh_final, aes(x = Position, fill = Status)) +
  ...
  scale_fill_manual(values=c("High"="#A2A4A3", "Medium"="#565A5C","Low"="#000000", "Routine"="#CFB87C")) +
  labs(
    title = "Athletes by Position + Watchlist Risk Status",
    x = "Positions",
    y = "Count",
    fill = "Risk Status"
  ) +
...
dash <- merge(dash, merge_lower1, by = c('anon_id', 'Group'), all = TRUE)
...
#creates a list of the anon ids that suffered HSI after being deemed low or high risk
false_ids <- c('ID_111', 'ID_117', 'ID_247', 'ID_430', 'ID_50', 'ID_75')

#filters for only these players
false_neg <- dash %>%
  filter(anon_id %in% false_ids) %>%
  transmute(`Player ID` = anon_id, Position, `Previous Injuries` = Prev.Injuries, `Percent Diff. Norm` = Perc.Diff.Norm, `Nordic Imbalance` = Nordic.Mean.Imbalance, Trend, Status, HSI = ifelse(HSI == 1, 'Yes', 'No')) %>%
  na.omit()

#creates a better looking table of these observations
formattable(false_neg, title = "Table of False Negatives")
```
...
incident_nohr <- filtered_incident %>%
...
incident_all <- filtered_incident %>%
...
In those not deemed high risk there are 235 athletes and only 14 unique athletes suffered HSI (0.05957447).
...
```{r}
df <- as.data.frame(as.table(table_2)) %>%
  mutate(Var1 = ifelse(Var1 == 'A', 'High Risk', 'Not High Risk'),
         Var2 = ifelse(Var2 == 'A', 'HSI', 'No HSI'))
colnames(df) <- c("Risk Determination", "Outcome", "Count")

ggplot(df, aes(x = `Risk Determination`, y = Count, fill = Outcome)) +
  geom_bar(stat = "identity", position = "fill") +
  labs(title = "Proportion of HSI in Unique Players by Risk Assessment (All Time)",
       y = "Proportion") +
  scale_fill_manual(values = c('#CFB87C', '#565A5C')) + 
  theme_bw()
```

When comparing the prevalance of HSI in high risk athletes vs the total population inluding the high risk athletes the p-value is 0.1411 meaning that there is not a ton of statistical evidence that the prevalance of HSI in high risk athletes is greater than the prevalance in the entire population.

When comparing the prevalance of HSI in high risk athletes vs all other athletes not deemed high risk the p-value is 0.003915 which shows high statistical evidence that the prevalance of HSI in righ risk athletes is greater than the prevalance of all other athletes. 
...
ggplot(DBs, aes(Nordic.MEAN.Imbalance, fill = as.factor(HR))) + 
  geom_histogram(position = 'identity', alpha = 0.5, bins = 25) + 
  scale_fill_manual(values = c('black', '#CFB87C')) + 
  labs(title = 'Nordic Imbalance vs High Risk Status (All Time) for DBs',
       x = 'Nordic Mean Imbalance (Absolute Value)',
       y = 'Number of DBs',
       fill = 'High Risk Status') + 
  geom_vline(xintercept = c(ID_111$Nordic.MEAN.Imbalance), col = 'orangered', linetype = 'dashed') + 
  theme_bw()
```

### 2. Percent Difference from Norm
...
```{r, warning = FALSE}
#creates a histogram of nordic mean imbalance for WRs filled by high risk status, position = 'identity' argument makes it so the bars do not stack and the vertical line is the example threshold found above
ggplot(DBs, aes(Perc.Diff.Norm, fill = as.factor(HR))) + 
  geom_histogram(position = 'identity', alpha = 0.5, bins = 25) + 
  scale_fill_manual(values = c('black', '#CFB87C')) + 
  labs(title = 'Percent Diff. Norm vs High Risk Status (All Time) for DBs',
       x = 'Percentage Difference from Norm',
       y = 'Number of DBs',
       fill = 'High Risk Status') + 
  geom_vline(xintercept = c(ID_111$Perc.Diff.Norm), col = 'orangered', linetype = 'dashed') + 
  theme_bw()
```
...
t.test(Trend.Num ~ HR, data = QBs, alternative = 'greater')
#t.test(Trend.Num ~ HR, data = RBs)
#t.test(Trend.Num ~ HR, data = WRs)
#t.test(Trend.Num ~ HR, data = TEs)
#t.test(Trend.Num ~ HR, data = OLs)
t.test(Trend.Num ~ HR, data = DBs, alternative = 'greater')
#t.test(Trend.Num ~ HR, data = LBs)
t.test(Trend.Num ~ HR, data = DLs, alternative = 'greater')
```
...
```{r, warning = FALSE}
#creates a histogram of nordic mean imbalance for WRs filled by high risk status, position = 'identity' argument makes it so the bars do not stack and the vertical line is the example threshold found above
ggplot(QBs, aes(Trend.Num, fill = as.factor(HR))) + 
  geom_histogram(position = 'identity', alpha = 0.5, bins = 3) + 
  scale_fill_manual(values = c('black', '#CFB87C')) + 
  labs(title = 'Strength Trend vs High Risk Status (All Time) for QBs',
       x = 'Strength Trend (-1:Down, 0:Neutral, 1:Up)',
       y = 'Number of QBs',
       fill = 'High Risk Status') + 
  theme_bw()
```
...
# Question 3: Exploring Other Associations / Defining Thresholds


## 1. Nordic Mean Imbalance Thresholds
...
```{r, warning = FALSE}
#creates a histogram of maximum force from norm for OLs filled by HSI status, position = 'identity' argument makes it so the bars do not stack and the vertical line is the example threshold found above
ggplot(OLs, aes(Maximum.Force, fill = as.factor(HSI))) + 
  geom_histogram(position = 'identity', alpha = 0.5, bins = 25) + 
  scale_fill_manual(values = c('black', '#CFB87C')) + 
  labs(title = 'Maximum Force vs HSI Status (All Time) for O-Linemen',
       x = 'Maximum Force (Nordic)',
       y = 'Number of O-Linemen',
       fill = 'HSI Status') + 
  theme_bw()
```
...
```{r, warning = FALSE}
#creates a histogram of percent difference from norm for RBs filled by HSI status, position = 'identity' argument makes it so the bars do not stack and the vertical line is the example threshold found above
ggplot(DBs, aes(Maximum.Nordic.Bilateral.Mean, fill = as.factor(HSI))) + 
  geom_histogram(position = 'identity', alpha = 0.5, bins = 25) + 
  scale_fill_manual(values = c('black', '#CFB87C')) + 
  labs(title = 'Maximum Force vs HSI Status (All Time) for RBs',
       x = 'Maximum Force (Nordic)',
       y = 'Number of RBs',
       fill = 'HSI Status') + 
  theme_bw()
...
```{r, warning = FALSE}
#creates a histogram of percent difference from norm for DBs filled by HSI status, position = 'identity' argument makes it so the bars do not stack and the vertical line is the example threshold found above
ggplot(DBs_c, aes(Max.Acceleration, fill = as.factor(HSI))) + 
  geom_histogram(position = 'identity', alpha = 0.5, bins = 25) + 
  scale_fill_manual(values = c('black', '#CFB87C')) + 
  labs(title = 'Maximum Force vs HSI Status (All Time) for O-Linemen',
       x = 'Maximum Force (Nordic)',
...
ggplot(RBs_w, aes(Soreness.Score, fill = as.factor(HSI))) + 
  geom_histogram(position = 'identity', alpha = 0.5, bins = 25) + 
  scale_fill_manual(values = c('black', '#CFB87C')) + 
  labs(title = 'Overall Wellness Score vs HSI Insidence (All Time) for RBs',
